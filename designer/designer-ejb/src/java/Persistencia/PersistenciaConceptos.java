...
public class PersistenciaConceptos implements PersistenciaConceptosInterface {
   ...
   public List<ConceptosAux2> buscarConceptosAux2(EntityManager em) {
      em.clear();
      try {
         Query query = em.createNativeQuery("SELECT (SELECT COUNT(*) FROM VIGENCIASCUENTAS VC, CUENTAS CD\n"
                 + " WHERE C.SECUENCIA = VC.CONCEPTO\n"
                 + " AND VC.CUENTAD = CD.SECUENCIA\n"
                 + " AND CD.CODIGO>0) VIGENCIASCUENTAS,\n"
                 + " SECUENCIA, DESCRIPCION, NATURALEZA, ACTIVO, CODIGO, CONJUNTO, EMPRESA FROM CONCEPTOS C\n"
                 + " ORDER BY CODIGO", ConceptosAux2.class);
         List<ConceptosAux2> listaConceptos = query.getResultList();
         return listaConceptos;
      } catch (Exception e) {
         log.error("Error en buscarConceptosAux2() :  ", e);
         return null;
      }
   }
   ...
   public int contarVigenciasCuentas(EntityManager em, BigInteger secuenciaConcepto) {
      em.clear();
      try {
         Query query = em.createNativeQuery("SELECT COUNT(*) conteo FROM VIGENCIASCUENTAS VC, CUENTAS CD\n"
                 + " WHERE ? = VC.CONCEPTO\n"
                 + " AND VC.CUENTAD = CD.SECUENCIA\n"
                 + " AND CD.CODIGO>0");
         ...
         log.error("Error en contarVigenciasCuentas() :  ", e);
         return 0;
      }
   }
