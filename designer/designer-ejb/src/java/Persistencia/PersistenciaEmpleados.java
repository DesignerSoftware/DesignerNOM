...
//   }
    ...
    @Override
    public List<Empleados> empleadosEmpresa(EntityManager em) {
        try {
            em.clear();
            String sql = "SELECT * FROM EMPLEADOS EM WHERE EXISTS (SELECT 'X'\n"
                    + "         FROM VIGENCIASTIPOSTRABAJADORES VTT, TIPOSTRABAJADORES TT\n"
                    + "         WHERE VTT.TIPOTRABAJADOR = TT.SECUENCIA\n"
                    + "         AND   VTT.EMPLEADO = EM.SECUENCIA\n"
                    + "         AND   (TT.TIPO = 'ACTIVO' OR TT.TIPO = 'PENSIONADO' )\n"
                    + "         AND   FECHAVIGENCIA = (SELECT MAX(FECHAVIGENCIA)\n"
                    + "                                FROM VIGENCIASTIPOSTRABAJADORES VTTI\n"
                    + "                                WHERE FECHAVIGENCIA<= (SELECT FECHAHASTACAUSADO FROM VWACTUALESFECHAS) AND \n"
                    + "VTT.EMPLEADO = VTTI.EMPLEADO))";
            Query query = em.createNativeQuery(sql, Empleados.class);
            listaEmpleados = query.getResultList();
            consultarTransientsLista(em, "SELECT E.SECUENCIA, P.PATHFOTO, P.NOMBRE NOMBREPERSONA, P.PRIMERAPELLIDO, P.SEGUNDOAPELLIDO,\n"
                    + " P.EMAIL, P.NUMERODOCUMENTO, EM.NOMBRE NOMBREEMPRESA, EM.RETENCIONYSEGSOCXPERSONA RETENCIONYSEGSOCXPERSONA\n"
