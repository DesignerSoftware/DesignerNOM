        ...
        try {
            ...
            if (listSNodos != null) {
                if (!listSNodos.isEmpty()) {
                    log.warn("resultado.size() : " + listSNodos.size());
                ...
                if (!listSNodos.isEmpty()) {
                    ...
                    for (int i = 0; i < listSNodos.size(); i++) {
                        ...
                        String stringSQLQuery = "SELECT sn.secuencia,\n"
                                ...
                                + " where sn.SECUENCIA = " + listSNodos.get(i).getSecuencia();
                        if (i == 0) {
//                     log.warn("stringSQLQuery : " + stringSQLQuery);
                        }
                        Query query2 = em.createNativeQuery(stringSQLQuery, SolucionesNodosAux.class);
                        SolucionesNodosAux sNAux = (SolucionesNodosAux) query2.getSingleResult();
                        listSNodos.get(i).setCodigoconcepto(sNAux.getCodigoconcepto());
        ...
        try {
            ...
            if (listSNodos != null) {
                if (!listSNodos.isEmpty()) {
                    ...
                    try {
                        while (!listSNodos.isEmpty()) {
                            log.warn("PersistenciaSolucionesNodos.solucionNodoCorteProcesoEmpleador() cod(0):" + listSNodos.get(0).getCodigoconcepto());
                            if (listSNodos.get(0).getCodigoconcepto() != null) {
                                List<SolucionesNodos> listaAux = new ArrayList<SolucionesNodos>();
                                int banderita = 0;
//                        for (int j = 0; j < listaReturn.size(); j++) {
    ...
    public BigDecimal diasProvisionados(EntityManager em, BigInteger secuencia, Date fechaContratacion) {
        try {
            ...
            Query query = em.createQuery("SELECT SN1.unidades "
                    ...
                    + " AND GC.empresa.secuencia=c.empresa) "
                    + " AND SN1.fechahasta = (SELECT MAX(SN2.fechahasta) "
                    + " FROM SolucionesNodos SN2 "
                    + " WHERE SN2.fechadesde>=:fechaContratacion "
            ...
            query.setParameter("fechaContratacion", fechaContratacion);
            BigDecimal llegada = (BigDecimal) query.getSingleResult();
            if (llegada == null) {
                llegada = new BigDecimal(0);
            }
            return llegada;
        } catch (Exception e) {
    ...
    public List<SolucionesNodos> solucionNodoEmpleado(EntityManager em, BigInteger secuenciaEmpleado) {
        try {
            ...
            if (listSNodos != null) {
                if (!listSNodos.isEmpty()) {
                    log.warn("listSNodos.size() : " + listSNodos.size());
                    for (int i = 0; i < listSNodos.size(); i++) {
                        em.clear();
                        String nitTercero = "NULL";
                        if (listSNodos.get(i).getNittercero() != null) {
            ...
            if (listSNodos != null) {
                if (!listSNodos.isEmpty()) {
                    ...
                    for (int i = 0; i < listSNodos.size(); i++) {
                        ...
                        SolucionesNodosAux sNAux = (SolucionesNodosAux) query2.getSingleResult();
                        if (i == 0) {
                            log.warn("Query: [" + stringSQLQuery + "]");
                            log.warn("sNAux: sNAux.getCodigocuentac(): " + sNAux.getCodigocuentac() + ", sNAux.getCodigocuentad(): " + sNAux.getCodigocuentad());
                        }
                        listSNodos.get(i).llenarTransients(sNAux);
                    }
                }
            }
            return listSNodos;
                ...
                if (!listSNodos.isEmpty()) {
                    ...
                    if (listSNodosAux != null) {
                        if (!listSNodosAux.isEmpty()) {
                            log.warn("listSNodosAux.size() : " + listSNodosAux.size());
