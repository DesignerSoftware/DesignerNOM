 ...
 * 'VWActualesNormasEmpleados' de la base de datos.
 *
 * @author betelgeuse
 */
@Stateless
   ...
   public VWActualesNormasEmpleados buscarNormaLaboral(EntityManager em, BigInteger secuencia) {
      try {
         ...
         if (e.getMessage().contains("did not retrieve any entities")) {
            log.trace("Error PersistenciaVWActualesNormasEmpleados.buscarNormaLaboral: " + e);
         } else {
            log.error("Error PersistenciaVWActualesNormasEmpleados.buscarNormaLaboral:  ", e);
         }
         return null;
      }
   }
