...
public class AdministrarInterfaseContableSapBOPQ implements AdministrarInterfaseContableSapBOPQInterface {
   ...
   private static Logger log = Logger.getLogger(AdministrarInterfaseContableSapBOPQ.class);
   ...
   public List<InterconSapBO> obtenerInterconSapBOPQParametroContable(Date fechaInicial, Date fechaFinal) {
      try {
         ...
         log.warn("Error obtenerInterconSapBOPQParametroContable Admi : " + e.toString());
         return null;
      }
   }
   ...
   public ActualUsuario obtenerActualUsuario() {
      try {
         ActualUsuario user = persistenciaActualUsuario.actualUsuarioBD(getEm());
         return user;
      } catch (Exception e) {
   ...
   public Date obtenerMaxFechaContabilizaciones() {
      try {
         Date fecha = persistenciaContabilizaciones.obtenerFechaMaximaContabilizacionesSAPBOV8(getEm());
         return fecha;
      } catch (Exception e) {
   ...
   public Date obtenerMaxFechaIntercoSapBO() {
      try {
         Date fecha = persistenciaInterconSap.obtenerFechaMaxInterconSAPBO(getEm());
         return fecha;
      } catch (Exception e) {
   ...
   public void actualizarFlagProcesoAnularInterfaseContableSAPBOPQ(Date fechaIni, Date fechaFin) {
      try {
         ...
         log.warn("Error actualizarFlagProcesoAnularInterfaseContableSAPBOPQ Admi : " + e.toString());
      }
   }
   ...
   public void cambiarFlagInterconContableSAPBOPQ(Date fechaIni, Date fechaFin, BigInteger proceso) {
      try {
         ...
         log.warn("Error cambiarFlagInterconContableSAPBOPQ Admi : " + e.toString());
      }
   }
   ...
   public void ejeuctarPKGUbicarnuevointercon_SAPBO_PQ(BigInteger secuencia, Date fechaIni, Date fechaFin, BigInteger proceso) {
      try {
         persistenciaInterconSap.ejeuctarPKGUbicarnuevointercon_SAPBOPQ(getEm(), secuencia, fechaIni, fechaFin, proceso);
      } catch (Exception e) {
         log.warn("Error ejeuctarPKGUbicarnuevointercon_SAPBO_PQ Admi : " + e.toString());
      }
   }
   ...
   public void ejecutarPKGCrearArchivoPlano(Date fechaIni, Date fechaFin, BigInteger proceso, String descripcion, String nombreArchivo) {
      try {
         persistenciaInterconSap.ejecutarPKGCrearArchivoPlanoSAPPQ(getEm(), fechaIni, fechaFin, proceso, descripcion, nombreArchivo);
      } catch (Exception e) {
