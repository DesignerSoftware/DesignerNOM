...
public class ControlDetalleCuenta implements Serializable {
   ...
   private static Logger log = Logger.getLogger(ControlDetalleCuenta.class);
...
//           ControlRastro controlRastro = (ControlRastro) fc.getApplication().evaluateExpressionGet(fc, "#{controlRastro}", ControlRastro.class);
   ...
   public void editarCelda() {
      if (cualTabla == 1) {
         ...
         log.info("editar celda credito");
         if (cuentaCreditoTablaSeleccionada != null) {
            log.info("entró al if de editarCelda Credito cualCeldaCredito : " + cualCeldaCredito);
...
//        }
   ...
   public void limpiarExportarD() {
      cuentaDebitoTablaSeleccionada = new VigenciasCuentas();
   }

   public void limpiarExportarC() {
      cuentaCreditoTablaSeleccionada = new VigenciasCuentas();
   }
   ...
   public List<VigenciasCuentas> getListCuentasCredito() {
      try {
         ...
         log.warn("Error getListCuentasCredito : " + e.toString());
         return null;
      }
   }
   ...
   public List<VigenciasCuentas> getListCuentasDebito() {
      try {
         ...
         log.warn("Error getListCuentasDebito : " + e.toString());
         return null;
      }
   }
