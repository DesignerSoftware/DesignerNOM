   ...
   public boolean validarCamposNulosEscalafonSalarial(int i) {
      ...
      if (i == 1) {
         ...
         if (nuevoEscalafonSalarial.getDescripcion() == null) {
            retorno = false;
         } else if (nuevoEscalafonSalarial.getDescripcion().isEmpty()) {
            retorno = false;
         }
      ...
      if (i == 2) {
         ...
         if (duplicarEscalafonSalarial.getDescripcion() == null) {
            retorno = false;
         } else if (duplicarEscalafonSalarial.getDescripcion().isEmpty()) {
            retorno = false;
         }
   ...
   public boolean validarCamposNulosGrupoSalarial(int i) {
      ...
      if (i == 0) {
         ...
         if (aux.getSalario() == null) {
            retorno = false;
         } else if (aux.getSalario().doubleValue() <= 0) {
            retorno = false;
         }
      }
      if (i == 1) {
         if (nuevoGrupoSalarial.getDescripcion() == null) {
            retorno = false;
         } else if (nuevoGrupoSalarial.getDescripcion().isEmpty()) {
            retorno = false;
         }
         if (nuevoGrupoSalarial.getSalario() == null) {
            retorno = false;
         } else if (nuevoGrupoSalarial.getSalario().doubleValue() <= 0) {
            retorno = false;
         }
      }
      if (i == 2) {
         if (duplicarGrupoSalarial.getDescripcion() == null) {
            retorno = false;
         } else if (duplicarGrupoSalarial.getDescripcion().isEmpty()) {
            retorno = false;
         }
         if (duplicarGrupoSalarial.getSalario() == null) {
            retorno = false;
         } else if (duplicarGrupoSalarial.getSalario().doubleValue() <= 0) {
            retorno = false;
         }
      }
      return retorno;
...
//      cancelarModificacionGeneral();
      ...
      if (confirmarCambio.equalsIgnoreCase("TIPOTRABAJADOR")) {
         ...
         if (coincidencias == 1) {
            if (tipoNuevo == 1) {
               ...
               RequestContext.getCurrentInstance().update("formularioDialogos:duplicarEscalafonSalarialTipoTrabajador");
            }
            lovTiposTrabajadores.clear();
            getLovTiposTrabajadores();
         } else {
   ...
   public void verificarRastroEscalafonSalarial() {
      ...
      } else if (administrarRastros.verificarHistoricosTabla("ESCALAFONESSALARIALES")) {
         nombreTablaRastro = "EscalafonesSalariales";
