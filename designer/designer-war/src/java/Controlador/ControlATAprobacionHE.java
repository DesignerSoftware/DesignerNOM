...
//           ControlRastro controlRastro = (ControlRastro) fc.getApplication().evaluateExpressionGet(fc, "#{controlRastro}", ControlRastro.class);
   ...
   public void modificarFechaCabecera(int i, int c) {
      ...
      if (retorno == true) {
         ...
         RequestContext.getCurrentInstance().update("form:tablaInferiorIzquierda");
         RequestContext.getCurrentInstance().update("form:tablaInferiorDerecha");
         RequestContext.getCurrentInstance().execute("PF('errorFechaCabecera').show()");
      }
   }
   ...
   public void editarCelda() {
      ...
      if (indexFlujo >= 0) {
         ...
         if (cualCeldaFlujo == 0) {
            ...
            RequestContext.getCurrentInstance().execute("PF('editarFlujoProceso').show()");
            cualCeldaFlujo = -1;
         }
         if (cualCeldaFlujo == 1) {
            RequestContext.getCurrentInstance().update("formularioDialogos:editarFlujoFecha");
            RequestContext.getCurrentInstance().execute("PF('editarFlujoFecha').show()");
            cualCeldaFlujo = -1;
         }
         indexFlujo = -1;
         secRegistro = null;
      ...
      if (indexDetalle >= 0) {
         ...
         if (cualCeldaDetalle == 0) {
            ...
            RequestContext.getCurrentInstance().execute("PF('editarDetalleConcepto').show()");
            cualCeldaDetalle = -1;
         }
         if (cualCeldaDetalle == 1) {
            RequestContext.getCurrentInstance().update("formularioDialogos:editarDetalleFecha");
            RequestContext.getCurrentInstance().execute("PF('editarDetalleFecha').show()");
            cualCeldaDetalle = -1;
         }
         if (cualCeldaDetalle == 2) {
            RequestContext.getCurrentInstance().update("formularioDialogos:editarDetalleObservacion");
            RequestContext.getCurrentInstance().execute("PF('editarDetalleObservacion').show()");
            cualCeldaDetalle = -1;
         }
         indexDetalle = -1;
         secRegistro = null;
   ...
   public void activarCtrlF11() {
      ...
      if (indexFlujo >= 0) {
         if (banderaFlujo == 0) {
            flujoFecha = (Column) c.getViewRoot().findComponent("form:datosFlujo:flujoFecha");
            flujoFecha.setFilterStyle("width: 85% !important;");
            flujoProceso = (Column) c.getViewRoot().findComponent("form:datosFlujo:flujoProceso");
            flujoProceso.setFilterStyle("width: 85% !important;");
            altoTablaFlujo = "88";
            RequestContext.getCurrentInstance().update("form:datosFlujo");
            banderaFlujo = 1;
      ...
      if (indexDetalle >= 0) {
         if (banderaDetalle == 0) {
            ...
            detalleFecha = (Column) c.getViewRoot().findComponent("form:datosDetalle:detalleFecha");
            detalleFecha.setFilterStyle("width: 85% !important;");
            detalleObservaciones = (Column) c.getViewRoot().findComponent("form:datosDetalle:detalleObservaciones");
            detalleObservaciones.setFilterStyle("width: 85% !important;");
            altoTablaFlujo = "63";
            RequestContext.getCurrentInstance().update("form:datosDetalle");
            banderaDetalle = 1;
   ...
   public void verificarRastroCabecera() {
      ...
      } else if (administrarRastros.verificarHistoricosTabla("EERSCABECERAS")) {
