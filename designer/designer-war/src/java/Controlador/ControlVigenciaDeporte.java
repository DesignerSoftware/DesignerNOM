...
public class ControlVigenciaDeporte implements Serializable {
   ...
   public void modificarVigenciaDeporte(VigenciasDeportes vigenciaDeportes, String confirmarCambio, String valorConfirmar) {
      ...
      if (confirmarCambio.equalsIgnoreCase("DEPORTES")) {
         ...
         if (coincidencias == 1) {
            if (tipoLista == 0) {
               vigenciaTablaSeleccionada.setDeporte(lovDeportes.get(indiceUnicoElemento));
            } else {
               vigenciaTablaSeleccionada.setDeporte(lovDeportes.get(indiceUnicoElemento));
            }
            lovDeportes.clear();
   ...
   public void autocompletarNuevoyDuplicado(String confirmarCambio, String valorConfirmar, int tipoNuevo) {
      ...
      if (confirmarCambio.equalsIgnoreCase("DEPORTES")) {
         ...
         if (coincidencias == 1) {
            if (tipoNuevo == 1) {
               nuevaVigenciaDeporte.setDeporte(lovDeportes.get(indiceUnicoElemento));
               RequestContext.getCurrentInstance().update("formularioDialogos:nuevaVigencias");
            } else if (tipoNuevo == 2) {
               duplicarVigenciaDeporte.setDeporte(lovDeportes.get(indiceUnicoElemento));
...
//RASTRO - COMPROBAR SI LA TABLA TIENE RASTRO ACTIVO
      ...
      try {
         if (listVigenciasDeportes == null) {
            if (empleado.getPersona() != null) {
               listVigenciasDeportes = administrarVigenciaDeporte.listVigenciasDeportesPersona(empleado.getPersona());
            }
         }
   ...
   public List<Deportes> getLovDeportes() {
      lovDeportes = administrarVigenciaDeporte.listDeportes();
      return lovDeportes;
   }

   public void setLovDeportes(List<Deportes> setListDeportes) {
      this.lovDeportes = setListDeportes;
   }
