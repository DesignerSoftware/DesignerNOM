...
//        }
   ...
   public void actionBtnGenerarPlano() {
      try {
         ...
         administrarInterfaseContableInfor.ejecutarPKGCrearArchivoPlano(parametroContableActual.getFechainicialcontabilizacion(), parametroContableActual.getFechafinalcontabilizacion(), parametroContableActual.getProceso().getSecuencia(), descripcionProceso, nombreArchivo);
         rutaArchivo = "";
   ...
   public void actionBtnRecontabilizar() {
      Integer contador = administrarInterfaseContableInfor.obtenerContadorFlagGeneradoFechasSAP(parametroContableActual.getFechainicialcontabilizacion(), parametroContableActual.getFechafinalcontabilizacion());
      if (contador != null) {
         if (contador != 0) {
   ...
   public void actionBtnEnviar() {
      ...
      if (fechaDesde != null && fechaHasta != null) {
         if ((fechaDesde.after(parametroContableActual.getFechainicialcontabilizacion()) && fechaHasta.after(parametroContableActual.getFechafinalcontabilizacion()))
            ...
            guardadoGeneral();
            administrarInterfaseContableInfor.ejeuctarPKGUbicarnuevointercon_Infor(parametroContableActual.getSecuencia(), parametroContableActual.getFechainicialcontabilizacion(), parametroContableActual.getFechafinalcontabilizacion(), parametroContableActual.getProceso().getSecuencia());
            listaGenerados = null;
            if (listaGenerados == null) {
               listaGenerados = administrarInterfaseContableInfor.obtenerSolucionesNodosParametroContable(parametroContableActual.getFechainicialcontabilizacion(), parametroContableActual.getFechafinalcontabilizacion());
               if (listaGenerados != null) {
...
//                    generarArchivoReporte(jp);
   ...
   public BigDecimal getTotalDInter() {
      ...
      if (listaInterconInfor != null) {
         for (int i = 0; i < listaInterconInfor.size(); i++) {
            totalDInter = totalDInter.add(listaInterconInfor.get(i).getValord());
         }
      }
      log.info("totalDInter : " + totalDInter);
      return totalDInter;
   ...
   public BigDecimal getTotalCInter() {
      ...
      if (listaInterconInfor != null) {
         for (int i = 0; i < listaInterconInfor.size(); i++) {
            totalCInter = totalCInter.add(listaInterconInfor.get(i).getValorc());
         }
      }
      log.info("totalCInter : " + totalCInter);
      return totalCInter;
