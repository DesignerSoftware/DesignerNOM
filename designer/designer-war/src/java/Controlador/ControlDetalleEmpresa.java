...
public class ControlDetalleEmpresa implements Serializable {
    ...
    private TercerosSucursales terceroSucursalLov;
    ...
    public void asignarIndex(DetallesEmpresas detalle, int dlg, int LND) {
        ...
        if (dlg == 0) {
            ...
            contarRegistrosTercerosSucursales();
    ...
    public void contarRegistrosTercerosSucursales() {
    ...
    public void actualizarTerceroSucursal() {
        RequestContext context = RequestContext.getCurrentInstance();
        if (tipoActualizacion == 0) {
            detalleSeleccionado.setEntidadsucursalarl(terceroSucursalLov.getSecuencia());
            detalleSeleccionado.setNombreEntidadArl(terceroSucursalLov.getDescripcion());
            if (!listDetallesEmpresasCrear.contains(detalleSeleccionado)) {
                if (listDetallesEmpresasModificar.isEmpty()) {
                    listDetallesEmpresasModificar.add(detalleSeleccionado);
                } else if (!listDetallesEmpresasModificar.contains(detalleSeleccionado)) {
        ...
        terceroSucursalLov = null;
        aceptar = true;
        tipoActualizacion = -1;
        RequestContext.getCurrentInstance().update("form:TercerosSucursalesDialogo");
        RequestContext.getCurrentInstance().update("form:lovTercerosSucursal");
        RequestContext.getCurrentInstance().update("form:aceptarTS");
        context.reset("form:lovTercerosSucursal:globalFilter");
        RequestContext.getCurrentInstance().execute("PF('lovTercerosSucursal').clearFilters()");
        RequestContext.getCurrentInstance().execute("PF('TercerosSucursalesDialogo').hide()");
    }

    public void cancelarCambioTerceroSucursal() {
        filtrarlovTercerosSucursales = null;
        terceroSucursalLov = null;
        aceptar = true;
        tipoActualizacion = -1;
    ...
    public List<TercerosSucursales> getLovTercerosSucursales() {
        if (lovTercerosSucursales == null) {
            lovTercerosSucursales = administrarDetalleEmpresa.lovTercerosSucursales();
        }
    ...
    public TercerosSucursales getTerceroSucursalLov() {
        return terceroSucursalLov;
    }

    public void setTerceroSucursalLov(TercerosSucursales terceroSucursalLov) {
        this.terceroSucursalLov = terceroSucursalLov;
    }
    ...
    public String getInfoRegistroTercerosSucursales() {
         FacesContext c = FacesContext.getCurrentInstance();
        DataTable tabla = (DataTable) c.getViewRoot().findComponent("form:lovTercerosSucursal");
