...
public class ControlJornadasLaborales implements Serializable {
   ...
   private static Logger log = Logger.getLogger(ControlJornadasLaborales.class);
...
//           ControlRastro controlRastro = (ControlRastro) fc.getApplication().evaluateExpressionGet(fc, "#{controlRastro}", ControlRastro.class);
   ...
   public void guardarTodo() {
      if (guardado == false) {
         log.info("Realizando Operaciones JornadasLaborales");
...
//Ubicacion Celda Arriba 
   public void cambiarJornadaLaboral() {
      ...
      //{
      log.info("secuencia jornada seleccionada: " + jornadaLaboralSeleccionada.getSecuencia());
      ...
      if (confirmarCambio.equalsIgnoreCase("N")) {
         ...
         if (!listaJornadasLaboralesCrear.contains(jornadaLaboralSeleccionada)) {
            ...
            if (jornadaLaboralSeleccionada.getCodigo() != null) {
               log.info("codiguin 1 " + codiguin);
               log.info("codiguin 1.2 " + jornadaLaboralSeleccionada.getCodigo());
      ...
      if (confirmarCambio.equalsIgnoreCase("N")) {
         ...
         if (!listaJornadasSemanalesCrear.contains(jornadaSemanalSeleccionada)) {
            ...
            if (aux.getMinutoinicial() == null) {
               log.info("Entro a Minuto Inicial");
               pasa++;
            }
            ...
            if (aux.getHorainicialalimentacion() == null) {
               log.info("Entro a Hora Inicial Alimentación");
            ...
            if (aux.getMinutoinicialalimentacion() == null) {
               log.info("Entro a Minuto Inicial Alimentación");
            ...
            if (aux.getHorafinalalimentacion() == null) {
               log.info("Entro a Hora Final Alimentación");
            ...
            if (aux.getMinutofinalalimentacion() == null) {
               log.info("Entro a Minuto Final Alimentación");
   ...
   public void seleccionarDia(String estadoDia, int indiceJS, int celda) {
      log.info("entre a modificar");
      int pasas = 0;
      int coincidencia = 0;

      //diarecuperado = jornadaSemanalSeleccionada.getDia();
      diita = jornadaSemanalSeleccionada.getEstadoDia();
      log.info("Diita: " + diita);
      //  log.info("Diarecuperado: " + diarecuperado);

      for (int i = 0; i < listaJornadasSemanales.size(); i++) {
         log.info("listaJornadasSemanales.get(i).getEstadoDia() Posicion : " + i + " " + listaJornadasSemanales.get(i).getEstadoDia());
         ...
         if (coincidencia > 1) {
            ...
            log.info("I find you 2!");
            log.info("dia 2: " + listaJornadasSemanales.get(i).getDia());
            log.info("estad dia 2: " + listaJornadasSemanales.get(i).getEstadoDia());
            RequestContext.getCurrentInstance().update("form:datosSemanasLaborales");
         }
      }

      if (jornadaSemanalSeleccionada.getDia() != null) {
         log.info("Dia en la Posicion: " + indiceJS + " " + jornadaSemanalSeleccionada.getDia());
   ...
   public void editarCelda() {
      if (jornadaLaboralSeleccionada != null && CualTabla == 0) {
         ...
         if (cualCelda == 0) {
            log.info("Codigo: " + editarJornadaLaboral.getCodigo());
   ...
   public void agregarNuevaSemanaLaboral() {
      ...
      if (nuevaSemanaLaboral.getDia() != null) {
         log.info("nuevaSemanaLaboral.getDia() : " + nuevaSemanaLaboral.getDia());
         for (int i = 0; i < listaJornadasSemanales.size(); i++) {
            log.info("listaJornadasSemanales.get(i).getDia() : " + listaJornadasSemanales.get(i).getDia());
      ...
      if (pasa == 0 && pasas == 0) {
         if (banderaJS == 1) {
            ...
            log.info("Desactivar");
            log.info("TipoLista= " + tipoListaJS);
            SemanaLaboralDia = (Column) c.getViewRoot().findComponent("form:datosSemanasLaborales:SemanaLaboralDia");
            SemanaLaboralDia.setFilterStyle("display: none; visibility: hidden;");
            SemanaLaboralHI = (Column) c.getViewRoot().findComponent("form:datosSemanasLaborales:SemanaLaboralHI");
            SemanaLaboralHI.setFilterStyle("display: none; visibility: hidden;");
...
//      RequestContext context = RequestContext.getCurrentInstance();
//      log.info("Cual Tabla= " + CualTabla);
//      log.info("listaJornadasLaborales: " + listaJornadasLaborales);
//      log.info("listaSemanas: " + listaJornadasSemanales);
...
//   }
   ...
   public void confirmarDuplicarJS() {
      ...
      if (duplicarSemanaLaboral.getDia() != null) {
         log.info("duplicarSemanaLaboral.getDia() : " + duplicarSemanaLaboral.getDia());
         for (int i = 0; i < listaJornadasSemanales.size(); i++) {
            log.info("listaJornadasSemanales.get(i).getDia() : " + listaJornadasSemanales.get(i).getDia());
   ...
   public void verificarRastro() {
      if (CualTabla == 0) {
         ...
         log.info("resultado: " + resultadoJS);
         if (resultadoJS == 1) {
