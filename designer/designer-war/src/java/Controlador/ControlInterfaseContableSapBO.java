...
@ManagedBean
@SessionScoped
public class ControlInterfaseContableSapBO implements Serializable {

   private static Logger log = Logger.getLogger(ControlInterfaseContableSapBO.class);
   ...
   private List<InterconSapBO> listaInterconSapBorrar;
   ...
   public ControlInterfaseContableSapBO() {
      ...
      listaInterconSapBorrar = new ArrayList<InterconSapBO>();
   ...
   public void modificarParametroContable(String confirmarCambio, String valorConfirmar) {
      ...
      if (valorConfirmar.equals("EMPRESA")) {
         ...
         if (!confirmarCambio.isEmpty()) {
            log.info("valor confirmar igual a proceso");
            log.info("parametroContableActual.getProceso().getDescripcion() : " + parametroContableActual.getProceso().getDescripcion());
...
//        }
   ...
   public void posicionIntercon() {
      ...
      int indice = Integer.parseInt(type);
      int columna = Integer.parseInt(name);
      interconTablaSeleccionada = listaInterconSapBO.get(indice);
   ...
   public String msnPaso1() {
      ...
      log.info("proceso en msnpaso 1 : " + parametroContableActual.getProceso());
   ...
   public void finCerrarPeriodoContable() {
      try {
         RequestContext context = RequestContext.getCurrentInstance();
         guardadoGeneral();
         administrarInterfaseContableSapBO.cerrarProcesoContabilizacion(parametroContableActual.getFechainicialcontabilizacion(), parametroContableActual.getFechafinalcontabilizacion(), parametroContableActual.getEmpresaCodigo(), parametroContableActual.getProceso().getSecuencia());
         listaGenerados = null;
   ...
   public void exportarPlano() throws IOException {
      try {
         log.info(this.getClass().getName() + ".exportarPlano()");
   ...
   public void actionBtnDeshacer() {
      try {
         RequestContext context = RequestContext.getCurrentInstance();
         guardadoGeneral();
         administrarInterfaseContableSapBO.actualizarFlagInterconSapBoProcesoDeshacer(parametroContableActual.getFechainicialcontabilizacion(), parametroContableActual.getFechafinalcontabilizacion(), parametroContableActual.getProceso().getSecuencia());
         administrarInterfaseContableSapBO.eliminarInterconSapBO(parametroContableActual.getFechainicialcontabilizacion(), parametroContableActual.getFechafinalcontabilizacion(), parametroContableActual.getEmpresaCodigo(), parametroContableActual.getProceso().getSecuencia());
         listaGenerados = null;
         if (listaGenerados == null) {
            listaGenerados = administrarInterfaseContableSapBO.obtenerSolucionesNodosParametroContable(parametroContableActual.getFechainicialcontabilizacion(), parametroContableActual.getFechafinalcontabilizacion());
            if (listaGenerados != null) {
   ...
   public void anularComprobantesCerrados() {
      try {
         guardadoGeneral();
         administrarInterfaseContableSapBO.actualizarFlagInterconSapBO(parametroContableActual.getFechainicialcontabilizacion(), parametroContableActual.getFechafinalcontabilizacion(), parametroContableActual.getEmpresaCodigo());
   ...
   public void validarFechasProcesoActualizar() {
      Date fechaContabilizacion = administrarInterfaseContableSapBO.obtenerMaxFechaContabilizaciones();
      Date fechaInterconSapBo = administrarInterfaseContableSapBO.obtenerMaxFechaIntercoSapBO();
      DateFormat df = DateFormat.getDateInstance(DateFormat.LONG);
      RequestContext context = RequestContext.getCurrentInstance();
      if (fechaContabilizacion != null && fechaInterconSapBo != null) {
         ...
         log.info("fecha InterconSa´BO en proceso actualizar" + fechaInterconSapBo);
   ...
   public void guardarCambiosParametro() {
      ...
      try {
         ...
         if (!listaGeneradosBorrar.isEmpty()) {
            administrarInterfaseContableSapBO.borrarRegistroGenerado(listaGeneradosBorrar);
            listaGeneradosBorrar.clear();
         }

         if (!listaInterconSapBorrar.isEmpty()) {
            administrarInterfaseContableSapBO.borrarRegistroIntercon(listaInterconSapBorrar);
   ...
   public void borrarRegistroGenerado() {
      log.info("entró a borrar registro generado");
   ...
   public void borrarRegistroContabilizado() {
      log.info("entró a borrar registro contabilizado");
      if (interconTablaSeleccionada != null) {
         ...
         listaInterconSapBO.remove(interconTablaSeleccionada);
         if (tipoListaIntercon == 1) {
            filtrarListaInterconSapBO.remove(interconTablaSeleccionada);
         }
   ...
   public void modificarTipoPlano(int tipo) {
      log.info("tipo : " + tipo);
      tipoPlano = tipo;
      log.info("tipo Plano despues de modificar : " + tipoPlano);
      guardadoGeneral();
   ...
   public void exportPDF_I() throws IOException {
      ...
      FacesContext context = c;
      Exporter exporter = new ExportarPDF();
      exporter.export(context, tabla, "InterconSapBO_PDF", false, false, "UTF-8", null, null);
      context.responseComplete();
      interconTablaSeleccionada = null;
   }
   ...
   public void exportXLS_I() throws IOException {
      ...
      FacesContext context = c;
      Exporter exporter = new ExportarXLS();
      exporter.export(context, tabla, "InterconSapBO_XLS", false, false, "UTF-8", null, null);
      context.responseComplete();
      interconTablaSeleccionada = null;
   }
...
//                    generarArchivoReporte(jp);
   ...
   public void validarDescargaReporte() {
      try {
         ...
         log.info("fecha desde : " + auxfechainicial);
         log.info("fecha hasta : " + auxfechafinal);
   ...
   public BigDecimal getTotalDInter() {
      ...
      if (listaInterconSapBO != null) {
         for (int i = 0; i < listaInterconSapBO.size(); i++) {
            totalDInter = totalDInter.add(listaInterconSapBO.get(i).getValord());
         }
      }
      log.info("totalDInter : " + totalDInter);
      return totalDInter;
   ...
   public BigDecimal getTotalCInter() {
      ...
      if (listaInterconSapBO != null) {
         for (int i = 0; i < listaInterconSapBO.size(); i++) {
            totalCInter = totalCInter.add(listaInterconSapBO.get(i).getValorc());
         }
      }
      log.info("totalCInter : " + totalCInter);
      return totalCInter;
